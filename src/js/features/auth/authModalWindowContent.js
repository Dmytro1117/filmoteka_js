import { singUp, singIn, logOut } from '../../api/firebase/firebaseApi';
...
export function showAuthorisedFields() {
  ...
  refs.autorization.style.display = 'none';
  refs.userStatus.textContent = `Hello, ${auth.currentUser.email}`;
...
export function showUnauthorisedFields() {
  ...
  refs.autorization.style.display = 'block';
  refs.userStatus.textContent = 'You are not logged in';
  const { loggedIn, notLoggedIn, userEmail } = dynRefs();
  if (notLoggedIn) {
